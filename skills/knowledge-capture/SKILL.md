...
- **Workflow-performance observation** (rework, rejected assumption, blocked investigation, missing
  ...
  one project.
- **Project Memory knowledge** → `project-memory/projects/<KEY>/knowledge/` (ARC/PAT/PIT/TRB).
- **Shared Brain candidate** → `shared-brain/engineering/` (SB-PAT/SB-PIT/SB-TRB/SB-DEC).
- **Decision** → `decision-capture` skill.
- **Stale knowledge correction** → mark the old entry superseded; add the correction.
...
override stored knowledge on conflict.
